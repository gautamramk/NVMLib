 ...
 * @param data: the `loop` that needs to be started
 * 
 ...
 * 
 * Here at first all the new objects created by memalloc() are recorded. Then 
 * all the objects are updated using the stats revieved from log_write() and log_read().
 * And finally the decision for movement of object is made.
 */
void on_logistics_timer(uv_timer_t *timer, int status) {
    uv_work_t* work_req;
 ...
 * 
 * @param key: The `MEMoidKey` of the object that needs to be deleted.
 * @param oid: The `MEMoid` object that represents the object that needs to be deleted.
 * @return Nothing.
 * 
 * @note This function is responsible for removing the entries about the *deleted object*
 * from the `object_maintainance table` and `types_table`.
 * 
 * @see on_deletion_timer()
 */
void delete_object(MEMoidKey key, MEMoid oid){
 ...
 * 
 * @param key: the `MEMoidKey` of the object that needs to be moved.
 * @param oid: the `MEMoid` object representing the object that needs to be moved.
 * @return Nothing.
 * 
 * @note This function has to update `object_maintainance table` and `types_table`
 * @attention This function has to ensure data integrety and pointer validity.
 * 
 * @see move_to_nvram() on_logistics_timer()
 */
void move_to_dram(uv_work_t *req) {
    MEMoidKey key = ((object_maintainance*)(req->data))->key;
    MEMoid oid = ((object_maintainance*)(req->data))->oid;
 ...
 * @attention This function has to ensure data integrety and pointer validity.
 * 
 * @see move_to_dram() on_logistics_timer()
 */
void move_to_nvram(uv_work_t *req) {
 ...
 * 
 * @param entry: The entry of the object under analysis from the `object maintainance table`.
 * @return 1: If needs to be moved from DRAM to NVRAM
 * @return 2: If needs to be moved from NVRAM to DRAM             
 * @return 0: otherwise
 */
int check_if_required_to_move(object_maintainance entry) {
    // To be completed
